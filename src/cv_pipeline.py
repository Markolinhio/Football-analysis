...
def box_intersection(bounding_box_1, bounding_box_2):
    dx = min(bounding_box_1[2], bounding_box_2[2]) - \
        max(bounding_box_1[0], bounding_box_2[0])
    dy = min(bounding_box_1[3], bounding_box_2[3]) - \
...
def in_possession(ball_box, player_box, intersection_threshold=10, distance_threshold=10):
    ...
    distance = np.linalg.norm(
        np.array(box_center(ball_box)) - np.array(box_center(player_box)))
...
def in_collision(player_box_1, player_box_2, intersection_threshold=10, distance_threshold=10):
    ...
    distance = np.linalg.norm(np.array(box_bottom_coord(
        player_box_1)) - np.array(box_bottom_coord(player_box_2)))
...
def check_collision(team_1_box, team_2_box, misc_box):
    ...
    team_1_uncollided = [a for a, b in zip(
        team_1_uncollided, team_2_uncollided) if not in_collision(a, b)]
    team_2_uncollided = [b for a, b in zip(
        team_1_uncollided, team_2_uncollided) if not in_collision(a, b)]

    # Check misc if collided with players
    misc_uncollided = [a for a, b in zip(
        misc_box, team_1_uncollided+team_2_uncollided) if not in_collision(a, b)]

    return team_1_uncollided, team_2_uncollided, misc_uncollided


def visualize_team_players(frame, team_1_box, team_2_box=None, misc_box=None, ball_box=None, team_1_ids=None, team_2_ids=None):
    temp = frame.copy()
